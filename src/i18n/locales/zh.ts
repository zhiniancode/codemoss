const zh = {
  ...
  sidebar: {
    ...
    openaiNewChat: "新建 Custom API 对话",
    openaiAddFolder: "添加文件夹 (Custom API)",
    openaiEmpty: "你可以直接开始 Custom API 对话（无项目），或添加一个文件夹作为上下文。",
  ...
  settings: {
    ...
    // 供应商管理
    vendorsTitle: "供应商管理",
    vendorsDescription: "管理 Claude、Codex 与 OpenAI-compatible 的 API 供应商配置。",
    vendor: {
      allProviders: "所有供应商",
      allCodexProviders: "所有 Codex 供应商",
      allOpenAIProviders: "所有 Custom API 供应商",
      add: "添加",
      edit: "编辑",
      delete: "删除",
      enable: "启用",
      ...
      emptyOpenAIState: "暂无 OpenAI-compatible 供应商，点击上方「添加」创建一个。",
      dialog: {
      ...
      openaiDialog: {
        addTitle: "添加 Custom API 供应商",
        editTitle: "编辑 Custom API 供应商",
        namePlaceholder: "例如: 我的 OpenAI 代理",
        baseUrl: "Base URL",
        baseUrlHint: "填写 OpenAI-compatible 的 base URL（通常以 /v1 结尾）。",
        apiKey: "API Key",
        defaultModel: "默认模型",
        defaultModelHint: "用于 OpenAI-compatible 引擎新会话的默认模型。",
        defaultModelPickPlaceholder: "点击检测模型",
        defaultModelNoModels: "未检测到模型（点击重试）",
        defaultModelManual: "手动输入模型 ID",
        defaultModelManualPlaceholder: "例如: gpt-4.1",
        models: "模型列表",
        autoDetect: "自动检测",
        detectModels: "检测模型",
        detecting: "检测中...",
        detectOk: "已发现 {{count}} 个模型（{{latency}} ms）",
        detectHint: "填写 Base URL 和 API Key 后会自动检测模型。",
        detectHintClickModel: "填写 Base URL 和 API Key 后，点击「默认模型」即可自动检测模型。",
        protocolMismatch: "检测到协议：{{protocol}}（不是 OpenAI Compatible）。",
        goToClaude: "切换到 Claude",
        goToClaudeHint: "这个地址看起来是 Anthropic/Claude 协议，请在 Claude 标签页配置。",
        geminiNotSupported: "这个地址看起来是 Gemini 协议，但当前这里还不支持。",
        fixedBaseUrlFound: "建议 Base URL：{{baseUrl}}",
        applyFixedBaseUrl: "应用",
        importModels: "导入模型",
        importModelsHint: "用检测到的模型列表替换下方的列表。",
        suggestClaude: "检测到 Anthropic/Claude 协议，建议切换到 Claude 供应商配置。",
        suggestGemini: "检测到 Gemini 协议，建议切换到 Gemini 供应商配置。",
        suggestCheckKey: "检测到的协议下 API Key 可能无效，请检查。",
      },
      deleteConfirm: {
        title: "确认删除",
